...
import { renderComposite, type Annotation, type Arrow } from '../components/renderComposite'
...
export default function Editor(): React.ReactElement {
  ...
  }, [img, imgSize, annotations, arrows])
  ...
  function onMouseUp(e: React.MouseEvent<HTMLCanvasElement>): void {
    ...
    if (dx < DRAG_THRESHOLD && dy < DRAG_THRESHOLD) { setDraft(null); return }
  ...
  const copyClose = useCallback(async () => {
    if (!img) return
    const dataUrl = await renderComposite(img, imgSize, annotations, arrows)
